/**
 * Session service for the API server.
 * Uses in-memory storage - persistence is handled by Electron.
 */
...
export type Session = {
  id: string;
  externalId?: string;  // ID from Electron for cross-process tracking
  title: string;
...
export async function initSessionStore(_dataDir: string): Promise<void> {
  ...
  console.log('Session store initialized (in-memory mode)');
}
